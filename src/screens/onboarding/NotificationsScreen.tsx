...
export default function NotificationsScreen() {
  ...
  const requestPermission = async () => {
    ...
    navigation.navigate('OnboardingMicrophone');
  };
  ...
  return (
    <View style={styles.container}>
        ...
        <SafeAreaView>
          <View style={styles.headerContent}>
            <View style={styles.stepIndicator}>
              {[1, 2, 3, 4].map((s) => (
                <View key={s} style={[styles.step, s <= 2 && styles.stepDone, s === 2 && styles.stepActive]} />
              ))}
            </View>
            <Text style={styles.stepText}>Step 2 of 5</Text>
            <Text style={styles.headerTitle}>{t('onboarding.enableNotifications')}</Text>
          </View>
        </SafeAreaView>
      </LinearGradient>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.title}>{t('onboarding.enableNotifications')}</Text>
        <Text style={styles.desc}>{t('onboarding.notificationsDesc')}</Text>

        <View style={styles.featuresList}>
          {[
            { icon: '🍼', text: 'Feeding reminders' },
            { icon: '😴', text: 'Sleep predictions' },
            { icon: '💉', text: 'Vaccine due alerts' },
            { icon: '🎯', text: 'Milestone updates' },
          ].map((f) => (
        ...
        <TouchableOpacity onPress={() => navigation.navigate('OnboardingMicrophone')} style={styles.skipBtn}>
...
const styles = StyleSheet.create({
  ...
  featuresList: { width: '100%', gap: Spacing.md, marginBottom: Spacing['2xl'] },
  featureItem: {
  ...
  featureText: { flex: 1, fontSize: Typography.base, color: Colors.textPrimary, fontWeight: '500' },
